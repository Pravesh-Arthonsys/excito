        ...
        {sortedProducts.map((product) => (
          <div key={product.id} className="group cursor-pointer">
            <div className="relative overflow-hidden rounded-lg bg-white shadow-sm hover:shadow-md transition-shadow">
              ...
              {product.is_limited_stock && product.stock_count && (
                <span className="absolute top-10 left-3 bg-orange-500 text-white px-2 py-1 rounded text-xs font-medium">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl p-6 max-w-sm w-full">
            <div className="flex justify-between items-start mb-4">
              <h3 className="font-bold text-gray-900">{quickAdd.product.name}</h3>
              <button onClick={() => setQuickAdd(null)} className="text-gray-400 hover:text-gray-600 cursor-pointer">
              ...
              <div className="flex flex-wrap gap-2">
                {quickAdd.product.sizes.map(s => (
                  <button key={s} onClick={() => setQuickAdd(prev => prev ? { ...prev, size: s } : null)}
                    className={`px-3 py-1.5 rounded-lg border text-sm font-medium cursor-pointer transition-colors ${quickAdd.size === s ? 'bg-black text-white border-black' : 'border-gray-300 text-gray-700 hover:border-black'}`}>{s}</button>
                ))}
              </div>
            </div>
            <div className="mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {quickAdd.product.colors.map(c => (
                  <button key={c} onClick={() => setQuickAdd(prev => prev ? { ...prev, color: c } : null)}
                    className={`px-3 py-1.5 rounded-lg border text-sm capitalize cursor-pointer transition-colors ${quickAdd.color === c ? 'bg-black text-white border-black' : 'border-gray-300 text-gray-700 hover:border-black'}`}>{c}</button>
                ))}
              </div>
            </div>
